<div class="col-md-12 locations form twelve columns nest">
<?php 
...
<div class="col-md-7 first seven columns">
...
<div class="col-md-5 last five columns text-right">
	...
	<?php // admin and super-admin can delete
		if($_SERVER['REMOTE_USER'] == 'hours' || $_SERVER['REMOTE_USER'] == 'hours_admin') {
			...
			echo $this->Html->link(__('Delete Date Range', true), array('action' => 'delete', $this->data['HourDateRange']['id']), array('class'=>'btn btn-danger'), sprintf(__('Are you sure you want to delete this date range?', true))); 
		}
	?> 
</div>
...
<?php	  
	...
	echo '<fieldset>';
	echo '<div class="col-md-2 first two columns"><label for="HourDateRangeBeginDateMonth">From</label></div>';
	echo '<div class="col-md-9 nine columns"><label for="HourDateRangeEndDateMonth">To</label></div>';
	echo '<div class="col-md-2 first two columns">';
	echo $this->Form->input('HourDateRange.begin_date',array('label'=>false,'type'=>'text','value'=>$begin_date, 'class'=>'form-control'));
	echo '</div>';	
	echo $this->Form->input('HourDateRange.orig_begin_date',array('type'=>'hidden','value'=>$origValues['HourDateRange']['begin_date'], 'class'=>'form-control'));
	echo '<div class="col-md-2 two columns">';
	echo $this->Form->input('HourDateRange.end_date', array('label'=>false,'type'=>'text','value'=>$end_date, 'class'=>'form-control'));
	echo $this->Form->input('HourDateRange.orig_end_date',array('type'=>'hidden','value'=>$origValues['HourDateRange']['end_date'], 'class'=>'form-control'));
